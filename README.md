...
**`client_unresponsive` also comes from the ticket's own Jira comments** (`src/lib/replyTracking.ts`), not only the SLA audit trail: 2+ follow-up rounds from us since the reporter last commented, the latest 4+ days old (`UNRESPONSIVE_MIN_FOLLOW_UPS` / `UNRESPONSIVE_MIN_DAYS` in `closureCandidates.ts`). This covers every open status, "Waiting for Product" included, and counts follow-ups posted directly in Jira, not just ones sent through this dashboard. Comments within 24h of each other count as one round (a message plus a quick correction is one follow-up, not two), and Jira automation (`app` accounts) never counts. Tickets with an open linked CP never reach this check (see below).

### Never Close While a CP Is Open

One rule shared by every closing path (`src/lib/linkedCp.ts`): if **any** linked CP is still open (Bug, Task, Story, Epic, any type), the ticket is never closed, never a Closure Candidate (not even a retry), and never flagged "Ready to close". It becomes closable only when there was never a CP and the client went unresponsive, or every linked CP is resolved (`linked_cp_resolved`, or the client then going quiet). The SLA cadence respects it too: while a CP is open, a ticket only gets a repeating stage-1 "still being worked on" check-in every 3 days (reason `cp_in_progress`, or `cp_not_worked` if nobody's picked the CP up, which keeps the SLA-breach Slack alert), never the stage-2 closing notice. Once the CP resolves, the closing step waits 3 days from the latest check-in. As a final safety net, `POST /api/tickets/[key]/followup/send` re-fetches the ticket and refuses (409, no comment posted) any closing send while a CP is open, whatever the browser asked for.
